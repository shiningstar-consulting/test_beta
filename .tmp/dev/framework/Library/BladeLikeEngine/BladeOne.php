...
class BladeOne
{
    ...
    public function showError(
        ...
        $msg =
            "<div style='background-color: red; color: black; padding: 3px; border: solid 1px black;'>";
    ...
    public function runString($string, $data = []): string
    {
        ...
        if ($previousError != $lastError && $lastError['type'] == E_PARSE) {
            ...
            $this->showError(
                'runString',
                $lastError['message'] . ' ' . $lastError['type'],
                true
            );
            return '';
        }
    ...
    public function compilePushOnce($expression): string
    {
        ...
        return $this->phpTag .
            "if(!isset($key)): $key=1;  \$this->startPush$expression; ?>";
    }
    ...
    protected function extendStartPush($section, $content): void
    {
        ...
        if (!isset($this->pushes[$section][$this->renderCount])) {
            ...
            $this->pushes[$section][$this->renderCount] =
                $content . $this->pushes[$section][$this->renderCount];
        }
    }
    ...
    {
        if ($name === 'if') {
            return $this->registerIfStatement(
                $args[0] ?? null,
                $args[1] ?? null
            );
        }
        $this->showError(
            'call',
            "function $name is not defined<br>",
            true,
            true
        );
        return '';
    ...
    public function runChild($view, $variables = []): string
    {
        if (\is_array($variables)) {
            ...
            $this->showError(
                'run/include',
                "RunChild: Include/run variables should be defined as array ['idx'=>'value']",
                true
            );
            return '';
        }
                ...
                if (is_callable($fn)) {
                    ...
                    if (method_exists($instance, 'composer')) {
                        ...
                        if ($this->mode === self::MODE_DEBUG) {
                            $this->showError(
                                'evalComposer',
                                "BladeOne: composer() added an incorrect method [$fn]",
                                true,
                                true
                            );
                            return;
                        ...
                        $this->showError(
                            'evalComposer',
                            'BladeOne: composer() added an incorrect method',
                            true,
                            true
                        );
                        return;
    ...
    public function getCompiledFile($templateName = ''): string
    {
        ...
        if ($style === 'auto') {
            $style =
                $this->getMode() === self::MODE_DEBUG ? 'nochange' : 'sha1';
        }
        switch ($style) {
            case 'normal':
    ...
    public function getTemplateFile($templateName = ''): string
    {
        ...
        $path = \implode('/', $arr);
        return $this->locateTemplate(
            $path . '/' . $file . $this->fileExtension
        );
    }
    ...
    public function getFile($fullFileName): string
    {
        ...
        $this->showError(
            'getFile',
            "File does not exist at paths (separated by comma) [$this->notFoundPath] or permission denied"
        );
        return '';
    }
    ...
    public function isExpired($fileName): bool
    {
        ...
        if (!isFile($template)) {
            if ($this->mode == self::MODE_DEBUG) {
                ...
                $this->showError(
                    'Read file',
                    'Template not found :' . $this->fileName,
                    true
                );
            }
        }
        ...
        if (!$this->compiledPath || !isFile($compiled)) {
            return true;
        }
    ...
    {
        if (is_array($varname)) {
            $this->variablesGlobal = \array_merge(
     ...
     * @return BladeOne
     */
    public function setCompileTypeFileName(
        string $compileTypeFileName
    ): BladeOne {
    ...
    {
        $length =
            \is_array($data) || $data instanceof Countable
                ? \count($data)
                : null;
    ...
    {
        return ($last = static::last($this->loopsStack))
            ? (object) $last
            : null;
    }
    ...
    public function run($view = null, $variables = []): string
    {
        ...
        if ($mode == 3) {
            $this->showError(
                'run',
                "we can't force and run fast at the same time",
                true
            );
        }
    ...
    public function endSlot(): void
    {
        ...
        $this->slots[$this->currentComponent()][$currentSlot] = \trim(
            \ob_get_clean()
        );
    }
    ...
    {
        return '<base rel="canonical" href="' .
            $this->phpTag .
    ...
    {
        return $this->phpTagEcho .
            "'<pre>'; var_dump$expression; echo '</pre>';?>";
    }
    ...
    protected function compileInject($expression): string
    {
        ...
        return $this->phpTag .
            "\$$var = \$this->injectClass('$namespace', '$var'); ?>";
    }
    ...
    protected function getEchoMethods(): array
    {
        $methods = [
            ...
            'compileEscapedEchos' => \strlen(
                \stripcslashes($this->escapedTags[0])
            ),
            'compileRegularEchos' => \strlen(
                \stripcslashes($this->contentTags[0])
            ),
        ];
    ...
    protected function compileStatementClass($match): string
    {
        if (isset($match[3])) {
            return $this->phpTagEcho .
                $this->fixNamespaceClass($match[1]) .
                $match[3] .
                '; ?>';
        }
    ...
    protected function compileStatementCustom($match): string
    {
        ...
        $v = $v == '' ? '' : ',' . $v;
        return $this->phpTag .
            'call_user_func($this->customDirectives[\'' .
            $match[1] .
            '\']' .
            $v .
            '); ?>';
    ...
    protected function compileRegularEchos($value): string
    {
        ...
        $callback = function ($matches) {
            ...
            return $matches[1]
                ? \substr($matches[0], 1)
                : $this->phpTagEcho . $wrapped . '; ?>' . $whitespace;
        };
        return \preg_replace_callback($pattern, $callback, $value);
    }
    ...
    protected function compileAuth($expression = ''): string
    {
        ...
        return $this->phpTag .
            "if(isset(\$this->currentUser) && \$this->currentRole==$role): ?>";
    }
    ...
    protected function compileElseAuth($expression = ''): string
    {
        ...
        return $this->phpTag .
            "elseif(isset(\$this->currentUser) && \$this->currentRole==$role): ?>";
    }
    ...
    protected function compileCan($expression): string
    {
        $v = $this->stripParentheses($expression);
        return $this->phpTag .
            'if (call_user_func($this->authCallBack,' .
            $v .
            ')): ?>';
    }
    ...
    protected function compileElseCan($expression = ''): string
    {
        ...
        if ($v) {
            return $this->phpTag .
                'elseif (call_user_func($this->authCallBack,' .
                $v .
                ')): ?>';
        }
    ...
    protected function compileCanAny($expression): string
    {
        $role = $this->stripParentheses($expression);
        return $this->phpTag .
            'if (call_user_func($this->authAnyCallBack,' .
            $role .
            ')): ?>';
    }
    ...
    protected function compileElseCanAny($expression): string
    {
        ...
        return $this->phpTag .
            'elseif (call_user_func($this->authAnyCallBack,' .
            $role .
            ')): ?>';
    }
    ...
    protected function compileError($expression): string
    {
        ...
        return $this->phpTag .
            '$message = call_user_func($this->errorCallBack,' .
            $key .
            '); if ($message): ?>';
    }
    ...
    protected function compileForeach($expression): string
    {
        ...
        return $this->phpTag .
            "$initLoop foreach(\$__currentLoopData as $iteration): $iterateLoop ?>";
    }
    ...
    {
        return $expression
            ? $this->phpTag . "if$expression break; ?>"
            : $this->phpTag . 'break; ?>';
    }
    ...
    {
        return $expression
            ? $this->phpTag . "if$expression continue; ?>"
            : $this->phpTag . 'continue; ?>';
    }
    ...
    protected function compileForelse($expression): string
    {
        ...
        return $this->phpTag .
            "$empty = true; foreach$expression: $empty = false; ?>";
    }
    ...
    {
        return $this->phpTag .
            "if (! empty(trim(\$this->yieldContent$expression))): ?>";
    }
    ...
    {
        return $this->phpTagEcho .
            "(isset(\$this->assetDict[$expression]))?\$this->assetDict[$expression]:\$this->baseUrl.'/'.$expression; ?>";
    }
    ...
    protected function compileJSon($expression): string
    {
        ...
        return $this->phpTagEcho .
            "json_encode($parts[0], $options, $depth); ?>";
    }
    //</editor-fold>
    ...
    public function checkHealthPath(): bool
    {
        ...
        if (is_dir($this->compiledPath)) {
            echo "Compile-path [$this->compiledPath] is a folder " .
                self::colorLog('OK') .
                "\n";
        } else {
            $status = false;
            echo "Compile-path [$this->compiledPath] is not a folder " .
                self::colorLog('ERROR', 'e') .
                "\n";
        }
        foreach ($this->templatePath as $t) {
            if (is_dir($t)) {
                echo "Template-path (view) [$t] is a folder " .
                    self::colorLog('OK') .
                    "\n";
            } else {
                $status = false;
                echo "Template-path (view) [$t] is not a folder " .
                    self::colorLog('ERROR', 'e') .
                    "\n";
            }
        }
        ...
        try {
            ...
            if ($f === false) {
                $status = false;
                $error = self::colorLog(
                    'Unable to create file [' . $this->compiledPath . '/dummy]',
                    'e'
                );
            }
            @unlink($rnd);
        ...
        echo 'Testing reading in the view folder [' .
            $this->templatePath[0] .
            "].\n";
    ...
    public function createFolders(): void
    {
        ...
        echo 'Creating compile folder[' .
            self::colorLog($this->compiledPath, 'b') .
            '] ';
        if (!\is_dir($this->compiledPath)) {
            $ok = @\mkdir($this->compiledPath, 0770, true);
            if ($ok === false) {
                echo self::colorLog(
                    "Error: Unable to create folder, check the permissions\n",
                    'e'
                );
            } else {
    ...
    public function cliEngine(): void
    {
        ...
        echo ' |  _ <| |/ _` |/ _` |/ _ \ |  | | \'_ \ / _ \\' . "\n";
        echo ' | |_) | | (_| | (_| |  __/ |__| | | | |  __/' . "\n";
        echo ' |____/|_|\__,_|\__,_|\___|\____/|_| |_|\___|' .
            ' V.' .
            self::VERSION .
            "\n\n";
        echo "\n";
        ...
        if (!$done) {
            echo " Syntax:\n";
            echo ' ' .
                self::colorLog('-templatepath', 'b') .
                " <templatepath> (optional) the template-path (view path).\n";
            echo "    Default model: 'views'\n";
            ...
            echo ' ' .
                self::colorLog('-compilepath', 'b') .
                " <compilepath>  (optional) the compile-path.\n";
            echo "    Default model: 'compiles'\n";
            ...
            echo ' ' .
                self::colorLog('-createfolder', 'b') .
                " it creates the folders if they don't exist.\n";
            echo "    Example: php ./vendor/bin/bladeonecli -createfolder\n";
            echo ' ' .
                self::colorLog('-clearcompile', 'b') .
                " It deletes the content of the compile path\n";
            echo ' ' .
                self::colorLog('-check', 'b') .
                " It checks the folders and permissions\n";
        }
    }
